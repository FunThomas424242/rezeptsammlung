...
public class CrawlerConfiguration {
    ...
    @Bean
    public FlatFileItemReader<URI> reader() {
        return new FlatFileItemReaderBuilder<URI>()
            .name("personItemReader")
            .resource(new ClassPathResource("sample-data.csv"))
            .delimited()
            .names(new String[]{"firstName", "lastName"})
            .fieldSetMapper(new BeanWrapperFieldSetMapper<URI>() {{
                setTargetType(URI.class);
            }})
            .build();
    }
    ...
    @Bean
    public Step step1(JdbcBatchItemWriter<Rezept> writer) {
